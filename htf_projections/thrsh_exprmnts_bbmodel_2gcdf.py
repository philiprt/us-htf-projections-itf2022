...
def manage_experiments(experiments):
    ...
    output_fname = sta_path + exp_str + "/bbmodel.pickle"
...
def thrsh_exprmnts_bbmodel(experiments):
    ...
    if mode == "multiprocess":
        ...
        print(sta["name"] + " (NOAA ID: " + str(sta["id"]) + ")", file=lf)
    ...
    fname = "../tides/output/" + sta_str + "/tide_99th_percentile_monthly.pickle"
    ...
    eta_99 = eta_99.loc[eta_99.index.month == iso_mo]
    ...
    dymx = dymx.loc[dymx.index.month == iso_mo]
    dycnt = dymx.groupby(pd.Grouper(freq=freq)).count().loc[eta_99.index]

    thrsh = np.arange(-300, 301)
    N_xd_tot = (dymx.values > thrsh[:, None]).sum(axis=1)
    thrsh1 = thrsh[np.argmax(N_xd_tot < 0.99 * dymx.dropna().size)]
    thrsh2 = thrsh[np.argmin(N_xd_tot > 0.01 * dymx.dropna().size)]
    thrsh = np.arange(thrsh1, thrsh2 + 1)
    ...
    frac = (x_nrm[1] - x_nrm[0]) * tg.index.year.unique().size / (10 * exp.index.size)
    ...
    with model:
        ...
        if mode == "multiprocess":
            ...
            trace = pm.sample(
                500, chains=2, cores=1, tune=2000, target_accept=0.9, progressbar=False,
            )

        elif mode == "serial":

            trace = pm.sample(500, chains=2, cores=2, tune=2000, target_accept=0.9)

        # -------------------------------------------------------------------

        # do not accept this fit if there are many divergences
        if np.sum(trace.get_sampler_stats("diverging")) > 200:
            raise Exception("Too many divergences.")

        # -------------------------------------------------------------------

        # extract values from trace for saving
        trace_vals = {v: trace.get_values(v) for v in trace.varnames}
