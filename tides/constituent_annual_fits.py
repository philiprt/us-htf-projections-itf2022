...
def constituent_annual_fits(
    sta, tg, cnst, out_path, fig_path, log_file, overwrite=False
):

    filename = out_path + "annual_tide_fits.pickle"

    if os.path.isfile(filename) and not overwrite:
        with open(filename, "rb") as f:
            previous_fit = pickle.load(f)
            tg["td2"] = previous_fit["td2"]
            annual_fits = previous_fit["annual_fits"]
        print("Annual tidal fits loaded.", file=log_file)
    ...
    print("Calculating annual tidal fits...", file=log_file)
    ...
    for y in yrs:
        ...
        if slyr.count() > 0.75 * 365 * 24:
            ...
            cidx = [
                np.where(coef[y].name == c)[0][0] if c in coef[y].name else None
                for c in cnst
            ]
            ...
            annual_fits["dgn"].loc[y, :] = [
                coef[y].diagn["PE"][ii] if ii is not None else None for ii in cidx
            ]
    ...
    with open(filename, "wb") as f:
        pickle.dump({"td2": tg.td2, "annual_fits": annual_fits}, f)
