...
def station_data(sta, reuse_acyc_fit=False):
    ...
    fname = "../data/tide_gauge/" + str(sta["id"]) + ".nc"
    ...
    if sta["id"] == "8545240":  # philadelphia
        tg.loc["1942", :] = None

    if sta["id"] == "9410170":  # san diego
        tg.loc["1919-07":"1919-12", :] = None
...
def initial_tide_fit(sta, tg, out_path, log_file, overwrite=False):

    out_name = out_path + "initial_tide_fit.pickle"

    if os.path.isfile(out_name) and not overwrite:
        with open(out_name, "rb") as f:
            init_fit = pickle.load(f)
            tg["utd_epch"] = init_fit["utd_epch"]
            tg["utd_epch_mnr"] = init_fit["utd_epch_mnr"]
            mjr_cnst = init_fit["mjr_cnst"]

        print("Initial tidal fit loaded.", file=log_file)
        return tg, mjr_cnst
    ...
    with open(out_name, "wb") as f:
        pickle.dump(
            {
                "utd_epch": tg["utd_epch"],
                "utd_epch_mnr": tg["utd_epch_mnr"],
                "mjr_cnst": mjr_cnst,
                "coef_epch": coef_epch,
            },
            f,
    ...
    print("Initial tidal fit calculated.", file=log_file)
...
def plot_gp_dist(
    ...
    if plot_samples:
        ...
        h_smpl = ax.plot(
            x, samples[:, idx], color=cmap(0.9), lw=0.5, alpha=0.2, **samples_kwargs
        )
