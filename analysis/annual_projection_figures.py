...
def annual_projections(
    ...
    prjn_subset = [
        analysis[loc["name"]][loc["scenario"]][loc["threshold"]] for loc in subset
    ]
    ...
    prjn_subset = [loc[quantity].loc[yr_lims[0] : yr_lims[1]] for loc in prjn_subset]
    ...
    for n, prjn in enumerate(prjn_subset):
        ...
        r = int(n / 2) + 1
        c = (n % 2) + 1
        traces = projection_traces(prjn, stn_meta[n], col, leg, show_tides_only)
        annotations.extend(projection_annotations(prjn, stn_meta[n], r, c, vspace, show_tides_only))
        for trc in traces:
...
def projection_traces(prjn, meta, col, leg, show_tides_only):
    ...
    if show_tides_only:
        ...
        prjn_traces.append({
            ...
            "showlegend": True if leg else False,
            "line": {"color": "slategray", "width": 3}, #, "dash": "dash"},
            "hoverinfo": "y",            
        })
...
def tides_only_projection(meta):
    ...
    fname = "../../../Data/TideGaugeSets/NOS/nos_hourly/" + str(meta["noaa id"]) + ".nc"
    ...
    sta_path = "../Output/json/" + str(meta["noaa id"]) + "/"
    ...
    fname = "../../Tides/NOAA/nos_21cent_tides/" + str(meta["noaa id"]) + ".nc"
