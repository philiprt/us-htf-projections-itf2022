...
def climatology_projections(
    analysis, subset, quantity, yoi_srch_rng, specific_years=None
):
    ...
    for n, prjn in enumerate(prjn_subset):
        ...
        annotations, shapes = projection_annotations(
            prjn, stn_meta[n], r, vspace, sph, col, specific_years
        )
        for trc in traces:
    ...
    fig.update_layout(
        ...
        height=800,
        template="none",
        margin=dict(l=55, r=180, b=25, t=85, pad=0,),
...
def projection_annotations(prjn, meta, r, vspace, sph, col, specific_years):
    ...
    for n, yr in enumerate(meta["steps"][1:]):
        txt = str(yr)
        if n == 0 and specific_years is None:
            txt += " (YOI)"
        annotations.append(
