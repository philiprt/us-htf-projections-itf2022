...
from multistation_analysis_figures import color_palette
...
def tide_gauge_data(sta):
    ...
    fname = "../../../Data/TideGaugeSets/NOS/nos_hourly/" + str(sta["noaa id"]) + ".nc"
...
def nodal_cycle_example():
    ...
    aN = tg.td.groupby(pd.Grouper(freq="A")).apply(lambda x: x.dropna().count())
    ...
    A_21 = np.stack(
        [np.sin(2 * np.pi * idx_21 / 18.61), np.cos(2 * np.pi * idx_21 / 18.61),]
    ).T
    ...
    fname = "../../../Data/Projections/NOAA_scenarios/NOAA_SLR_scenarios.pickle"
    with open(fname, "rb") as f:
...
def nodal_cycle_figure(stations, combine_regions=None):
    ...
    fig = px.scatter(
        ...
        color_discrete_sequence=[
            px.colors.qualitative.Bold[n]
            for n in [0, 1, 2, 3, 10, 4, 6]
    ...
    fig.for_each_trace(
        lambda trace: trace.update(hovertemplate=hover_template(trace.showlegend),)
    )
    fig.add_scatter(
        x=[-1, 10],
        y=[0.5, 0.5],
    ...
    annotations = [
        ...
        {**{"x": 0.0, "y": 1.0275, "text": "NOAA Intermediate SLR scenario",}, **cmmn,},
        {**{"x": 0.0, "y": 0.975, "text": "St. Petersburg, FL",}, **cmmn,},
    ]
    ...
    fig.update_layout(
        ...
        width=800,
        height=450,
        margin=dict(l=70, r=15, b=80, t=40, pad=0,),
        xaxis=dict(
    ...
    fig.add_scatter(
        x=noaa_prjn.index,
        y=noaa_prjn.values,
        line=dict(color=fcol[0], width=0),
        xaxis="x2",
        yaxis="y2",
        showlegend=False,
    )
    ...
    annotations = annotations + [
        ...
        #         },
        {
            **{
                "x": 2022.75,
                "y": 7.3,
                "text": np.round(ncyc_21.max() - ncyc_21.min(), 1).astype(str) + " cm",
            },
            **cmmn,
        },
        {
            **{
                "x": 2034.5,
                "y": -6.5,
                "text": noaa_prjn.loc[2024:2034].iloc[[0, -1]].diff().iloc[-1].round(1)
                #                 .astype(int)
                .astype(str) + " cm",
            },
            **cmmn,
        },
        {
            **{
                "x": 2044.5,
                "y": 3.5,
                "text": noaa_prjn.loc[2034:2044].iloc[[0, -1]].diff().iloc[-1].round(1)
                #                 .astype(int)
                .astype(str) + " cm",
            },
            **cmmn,
        },
        {
            **{
                "x": 2034.5,
                "y": 25.5,
                "text": tot.loc[2024:2034].iloc[[0, -1]].diff().iloc[-1].round(1)
                #                 .astype(int)
                .astype(str) + " cm",
            },
            **cmmn,
        },
        {
            **{
                "x": 2044.5,
                "y": 35,
                "text": tot.loc[2034:2044].iloc[[0, -1]].diff().iloc[-1].round(1)
                #                 .astype(int)
                .astype(str) + " cm",
            },
            **cmmn,
